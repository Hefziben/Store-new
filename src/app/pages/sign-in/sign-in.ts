  ...
  onFacebookButtonTouched() {
    ...
    if (!this.isHybrid()) {
      this.userService.loginViaFacebook()
        ...
        .catch(e => console.log('Error logging to Facebook', e.errorMessage));
    } else {
      this.fb.login(['email','public_profile']).then((res: FacebookLoginResponse) => this.loggedIntoFacebook(res)).catch(e => console.log('Error logging at Facebook', e.errorMessage));
    }
